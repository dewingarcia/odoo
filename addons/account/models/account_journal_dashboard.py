...
class account_journal(models.Model):
    ...
    def get_line_graph_datas(self):
        ...
        # Return a list containing the latest bank statement lines balance per day for the
        # last 30 days for current journal
        query = """SELECT date, sum(amount) as amount_total
                        ...
                        GROUP BY date
                        ORDER BY date;"""
        self.env.cr.execute(query, (','.join(map(str, self.ids)), last_month, today))
        bank_stmt_lines = self.env.cr.dictfetchall()
        last_bank_stmt_line = self.env['account.bank.statement.line'].search([('journal_id', 'in', self.ids),('date', '<', last_month.strftime(DF))], order="date desc, id desc")
        start_balance = sum(last_bank_stmt_line.mapped('amount'))
        ...
        for stmt in bank_stmt_lines:
            ...
            #add new stmt value
            data[len(data) - 1]['y'] = last_balance + stmt.get('amount_total')
