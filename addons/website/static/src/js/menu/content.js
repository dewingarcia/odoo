odoo.define("website.contentMenu.instance", function (require) {
    "use strict";
    ...
    WebsiteNavbar.include({
        custom_events: _.extend({}, WebsiteNavbar.prototype.custom_events || {}, {
            edit_navbar_menu: function (e) {
                var result = this.contentMenu.edit_menu.apply(this.contentMenu, e.data.args || []);
                if (e.data.then) result.then(e.data.then);
            },
        }),
        start: function () {
            this.contentMenu = new ContentMenu(this);
            return $.when(
                this._super.apply(this, arguments),
                this.contentMenu.attachTo(this.$(".oe_content_menu"))
            );
        }
    });
});
...
odoo.define('website.contentMenu', function (require) {
    ...
    return Widget.extend({
        start: function () {
            // Add menu content event
            var self = this;
            this.$el.on('click', 'a[data-action]', function (e) {
                e.preventDefault();
