...
class WebsiteRatingProject(http.Controller):
    ...
    def page(self, project_id=None, **kw):
        ...
        values = {
            'project': project,
            'partner_task_rating': request.env['project.rating']._get_partner_rating("project_task", "project.task", project_id, limit=40),
            'partner_issue_rating': request.env['project.rating']._get_partner_rating("project_issue", "project.issue", project_id, limit=40),
        }
