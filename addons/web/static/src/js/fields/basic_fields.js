...
var StatusBar = AbstractField.extend({
    ...
    render: function() {
        ...
        var content = qweb.render("FieldStatus.content.desktop", {'widget': this});
        self.$el.html(content);
    },
    // FIXME: sometimes, the statusbar should not be clickable !
    // FIXME: the widget should be disabled once a stage has been clicked, and re-rendered when
    // the change has been written in DB (in readonly), or directly (in edit)
    on_click_stage: _.debounce(function(event) {
    ...
    render: function() {
        ...
        switch (this.node_options.terminology) {
            ...
            default:
                ...
                text = this.value ? _t(terms.string_true) || _t("On")
                                  : _t(terms.string_false) || _t("Off");
                hover = this.value ? _t(terms.hover_true || terms.string_false) || _t("Switch Off")
                                   : _t(terms.hover_false || terms.string_true) || _t("Switch On");
        }
