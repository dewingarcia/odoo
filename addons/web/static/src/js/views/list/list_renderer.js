...
return AbstractRenderer.extend({
    ...
    init: function(parent, arch, state, options) {
        options = options || {};
        this._super(parent, arch, state);
        ...
        this.mode = this.arch.attrs.editable ? "edit" : "readonly";
    },
    update: function (data, selection) {
        this.selection = selection || [];
        // todo: remove the reference to relational_data...
        // it should only use relational_data contained in data
        this.relational_data = data.relational_data;
    ...
    _render_header_cell: function(node) {
        ...
        if (node.attrs.widget) {
            var Widget = field_registry.get(node.attrs.widget);
    ...
    _render_body_cell: function(record, node) {
        ...
        if (node.attrs.widget) {
            var Widget = field_registry.get_any(['list.' + node.attrs.widget, node.attrs.widget]);
            if (Widget) {
    ...
    _render_footer: function(is_grouped) {
        ...
        var $cells = this._render_aggregate_cells(aggregates);
        if (is_grouped) {
            $cells.unshift($('<td>'));
        }
        if (this.has_selectors) {
        ...
        _.each(this.columns, function (column) {
            ...
            if (func) {
                ...
                _.each(data, function (d) {
                    count += 1;
                    var value = d.is_record ? d.data[attrs.name] : d.aggregate_values[attrs.name];
                    if (func === 'avg') {
    ...
    _render_group: function(group) {
        ...
        var field = this.state.fields[group.grouped_by[0]];
    ...
    _render_aggregate_cells: function (aggregate_values) {
        ...
        return _.map(this.columns, function(column) {
            ...
            if (column.attrs.name in aggregate_values) {
                ...
                $cell.addClass('o_list_number').attr('title', help).html(formatted_value);
            }
            return $cell;
        });
    },
