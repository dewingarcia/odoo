...
var ActionManager = Widget.extend(mixins.CacheMixin, mixins.UtilsMixin, {
    ...
    // perform an action.
    // params:
    // * action: the description of an action.  It can take one of several form:
    //   - object: It is the record ir.actions.act_window, with most(?) fields
    //   - number: it should be the id of an ir.action.  In that case, there should
    //     be an 'additional_context' key in options (??)
    // * options: object, with
    //   - action_menu_id: ...
    //   - clear_breadcrumbs: ...
