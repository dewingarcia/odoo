...
/**
 * The DomainNode Widget is an abstraction for widgets which can represent and
 * allow edition of a domain (part).
 */
var DomainNode = Widget.extend({
    ...
    /**
     * @constructor
     * A DomainNode needs a model and domain to work. It can also receives a set
     * of options.
     *
     * @param {Object} parent
    ...
    /**
     * The getDomain method is an abstract method which should returns the
     * prefix domain the widget is currently representing (an array).
     *
     * @return {Array}
     */
    getDomain: function () {},
...
/**
 * The DomainTree is a DomainNode which can handle subdomains (a domain which is
 * composed of multiple parts). It thus will be composed of other DomainTree
 * instances and/or leaf parts of a domain (@see DomainLeaf).
 */
var DomainTree = DomainNode.extend({
     ...
     * operator from the domain (default to "&").
    ...
    /**
     * The changeOperator method changes the operator of the domain tree and
     * notifies the parent if necessary (not silent).
     *
     * @param {string} operator - the new operator
     * @param {boolean} silent - true if the parents should not be notified of
     *                         the change
     */
    changeOperator: function (operator, silent) {
     ...
     *
     * @return {boolean} true if the child was removed, false otherwise (the
    ...
    /**
     * The @private _addFlattenedChildren method adds a child which represents
     * the given domain. If the child has children and that the child main
     * domain operator is the same as the current widget one, the 2-children
     * prefix hierarchy is then simplified by making the child children the
     * widget own children.
     *
...
var DomainSelector = DomainTree.extend({
    ...
    /**
     * The setDomain method changes the internal domain value and forces a
     * reparsing and redraw. If the internal domain value was already equal to
     * the given one, this does nothing.
     * @param {(Array|string)} domain [description]
     */
    ...
    /**
     * Called when the "add a filter" button is clicked -> add a first domain
     * node
     */
    ...
    /**
     * Called when the debug input value is changed -> construct the tree
...
/**
 * The DomainLeaf widget is a DomainNode which handles a domain which cannot be
 * split in another subdomains, i.e. composed of a field chain, an operator and
 * a value.
 */
var DomainLeaf = DomainNode.extend({
    ...
    /**
     * The willStart method prepares the information the rendering of the widget
     * will need by pre-instantiating its internal field selector widget.
     *
     * @return {Deferred}
     */
    willStart: function () {
        ...
        defs.push(this.fieldSelector.appendTo($("<div/>")).then((function () {
            ...
            if (!this.readonly) {
                ...
                try {
                    if (!selectedField.relation) { // TODO in this case, the value should be m2o input, etc...
                ...
                if (_.contains(["date", "datetime"], selectedField.type)) {
                    ...
                    wDefs.push(this.valueWidget.appendTo("<div/>").then((function () {
                        ...
                        this.valueWidget.on("datetime_changed", this, function () {
                            this.changeValue(this.valueWidget.get_value());
                        });
                    }).bind(this)));
                }
    ...
    /**
     * @see DomainNode.start
     * Append the prepared field selector and value widget.
     *
     * @return {Deferred}
     */
    start: function () {
    ...
    /**
     * The changeFieldChain method handles a field chain change in the domain.
     * In that case, the operator should be adapted to a valid one for the new
     * field and the value should also be adapted to the new field and/or
     * operator.
     *
     * -> trigger_up domain_changed event to ask for a re-rendering (if not silent)
     *
     ...
     */
    changeFieldChain: function (chain, silent) {
    ...
    /**
     * The changeOperator method handles an operator change in the domain. In
     * that case, the value should be adapted to a valid one for the new
     * operator.
     *
    ...
    /**
     * The changeValue method handles a formatted value change in the domain.
     * In that case, the value should be adapted to a valid technical one.
     *
     ...
     *                         domain_changed event
     */
    changeValue: function (value, silent) {
        ...
        if (selectedField.type === "boolean") {
            ...
            if (couldNotParse || _.isBoolean(this.value)) {
                this.value = field_utils.parse[selectedField.type](field_utils.format[selectedField.type](Date.now(), selectedField), selectedField);
            }
        } else {
    ...
    /**
     * The @private _getOperatorsFromType returns the mapping of "technical
     * operator" to "display operator value" of the operators which are
     * available for the given field type.
     *
     * @param {string} type - the field type
     *
     * @return {Object} a map of all associated operators and their label
     */
    ...
    _onOperatorSelectChange: function (e) {
        this.changeOperator($(e.currentTarget).val());
    },
    /**
    ...
    _onValueInputChange: function (e) {
        ...
        this.changeValue($(e.currentTarget).val());
    },
    /**
    ...
    _onFieldChainChange: function (e) {
        this.changeFieldChain(e.data.chain);
    },

    // TODO These two should be in an independant widget
    on_add_tag: function (e) {
        ...
        if (val && values.indexOf(val) < 0) {
            values.push(val);
            _.defer(this.changeValue.bind(this, values));
            $input.focus();
        }
    },
...
/**
 * The instantiateNode function instantiates a DomainTree if the given domain
 * contains several parts and a DomainLeaf if it only contains one part.
