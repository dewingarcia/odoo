...
class WebsiteSale(http.Controller):
    ...
    def access_abaundand_cart(self, token=None, **post):
        """
        Token will be pass from revival email template and it will redirect to the selected cart.
        """
        order = request.env['sale.order'].sudo().search([('cart_access_token', '=', token)], limit=1)
        if not order:
        ...
        if request.session.get('sale_order_id') and order.id != request.session['sale_order_id']:
            response = self.cart()
            response.qcontext.update({'token': order.cart_access_token})
            return response
        request.session['sale_order_id'] = order.id
        return request.redirect("/shop/cart")
