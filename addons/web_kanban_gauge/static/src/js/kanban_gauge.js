...
var GaugeWidget = AbstractField.extend({
    ...
    render: function() {
        // parameters
        var max_value = this.node_options.max_value || 100;
        if (this.node_options.max_field) {
            max_value = this.record_data[this.node_options.max_field];
        }
        var label = this.node_options.label || "";
        if (this.node_options.label_field) {
            label = this.record_data[this.node_options.label_field];
        }
        var title = this.node_options.title || this.field.string;
        // current gauge value
        var val = this.value;
        ...
        if (this.node_options.gauge_value_field) {
            gauge_value = this.record_data[this.node_options.gauge_value_field];
        }
        ...
        this.$el.empty().attr('style', this.node_options.style + ';position:relative; display:inline-block;');
