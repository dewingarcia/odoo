...
class Department(models.Model):
    ...
    allocation_requests_count = fields.Integer(
        string='Number of Allocations to Approve', compute='_compute_requests') # duplicate with above ... but above doesn't consider those in validate1 ...
...
class Employee(models.Model):
    ...
    leaves_count = fields.Integer('Number of Leaves', compute='_compute_leaves_count') # same field as remaining leaves, only doesn't bypass access rights...
    ...
    def _get_remaining_leaves(self):
        ...
        self._cr.execute("""
            SELECT
                sum(h.number_of_days - coalesce(hres.number_of_days, 0)) AS days,
                h.employee_id
            FROM
                hr_holidays h
